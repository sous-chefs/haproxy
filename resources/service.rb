    ...
    with_run_context(:root) do
      if %i(start restart reload).include?(resource_action)
        begin
          ...
          declare_resource(:systemd_unit, "#{new_resource.service_name}.service").delayed_action(resource_action)
...
action :create do
  with_run_context :root do
    declare_resource(:file, '/etc/default/haproxy') do
      content ''
      owner 'root'
      group 'root'
      mode '0644'
...
action :delete do
  with_run_context :root do
    declare_resource(:file, '/etc/default/haproxy').action(:delete)
    declare_resource(:systemd_unit, "#{new_resource.service_name}.service").action(%i(stop disable delete))
  end
end
